...
Nhập STT các loại NV cần làm (ví dụ: 12 để làm cảm xúc và comment): ''',end='')
    ...
    while demht < SO_NV1:
        try:
            ...
            if not JOB:
                prints(255,255,0,'Không tìm thấy nhiệm vụ, chờ 10 giây...', end='\r')
                time.sleep(10)
                continue
...
if __name__ == "__main__":
    try:
        # Chạy phần xác thực đã được đơn giản hóa


        # Gọi thẳng công cụ chính mà không cần kiểm tra kết quả xác thực
        main_bumx_free()
            
    except Exception as e:
        # Bắt tất cả các lỗi khác để tránh lộ code
        print("đang lỗi, vui lòng chờ….")
        time.sleep(5)
        sys.exit(1)
