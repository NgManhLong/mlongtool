...
def Delay(value):
    while not(value <= 1):
        value -= 0.123
        print(f'''{v}[{xanh}TOOL-TTC{v}] [{xanh}DELAY{v}] [{xanh}{str(value)[0:6]}{v}] [{vang}X    {v}]''', '           ', end = '\r')
        sleep(0.02)
        print(f'''{v}[{xanh}TOOL-TTC{v}] [{xanh}DELAY{v}] [{xanh}{str(value)[0:6]}{v}] [ {vang}X   {v}]''', '           ', end = '\r')
        sleep(0.02)
        print(f'''{v}[{xanh}TOOL-TTC{v}] [{xanh}DELAY{v}] [{xanh}{str(value)[0:6]}{v}] [  {vang}X {v}]''', '            ', end = '\r')
...
def addcookie():
    ...
    while True:
        ...
        if 'success' in info:
            ...
            print(f'{do}Cookie die hãy nhập lại cookie ')
            i -= 1
os.system("cls" if os.name == "nt" else "clear")
# banner()
...
if os.path.exists(f'cookiefb-ttc.json') == False:
    ...
    print(f'{do}══════════════════════════════════════════════════════  ')
    chon = input(f'{thanh}{luc}mời chọn{trang}: {vang}')
    print(f'{do}══════════════════════════════════════════════════════  ')
    while True:
        if chon == '1':
...
os.system("cls" if os.name == "nt" else "clear")            
banner()
print(f'{thanh}{luc}tên Tương Tác Chéo{trang}: {vang}{users}')
print(f'{thanh}{luc}số xu{trang}: {vang}{str(format(int(checktoken['xu']),","))}')
                ...
                if "idpost" in getjob.text or "idfb" in getjob.text:
                    ...
                    for x in getjob.json():
                        ...
                        if nhanxu.get('status') == 'success':
                            ...
                            print(f'{do}[ \033[1;36m{stt}{do} ] {do}[ {vang}NML{do} ][ {xanh}{timejob}{do} ][ {vang}{type.upper()}{do} ][ {trang}{id_}{do} ][ {vang}{msg}{do} ][ {luc}{str(format(int(xu),","))} {do}]')
                            if stt % 10 == 0:
                            ...
                            JobFail += 1
                            print(f'{trang}[{do}{JobFail}{trang}] {trang}[{do}JOB LỖI{trang}] {trang}{id_}','            ',end="\r")
