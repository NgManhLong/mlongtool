...
def banner():
    ...
    tool = "TRAO ĐỔI SUD FB - TOKEN VER"
    print(f'''
...
{logo} {t}Phiên Bản{d} :{l} Python 3.12 (API TOKEN)
...
def Delay(value):
	while not(value <= 1):
		value -= 0.123
		print(f'''{trang}[{lam}NML{trang}] [{lam}DELAY{trang}] [{lam}{str(value)[0:5]}{trang}] [{vang}X    {trang}]''', '               ', end = '\r')
		sleep(0.02)
		print(f'''{trang}[{lam}NML{trang}] [{lam}DELAY{trang}] [{lam}{str(value)[0:5]}{trang}] [ {vang}X   {trang}]''', '               ', end = '\r')
		sleep(0.02)
		print(f'''{trang}[{lam}NML{trang}] [{lam}DELAY{trang}] [{lam}{str(value)[0:5]}{trang}] [  {vang}X  {trang}]''', '               ', end = '\r')
...
class Facebook_Api(object):
    ...
    def info(self):
        get = get = requests.get('https://www.facebook.com/me', headers=self.headers, proxies=self.proxies).text
...
# --- CLASS TRAODOISUB API ĐÃ SỬA ĐỔI ---
class TraoDoiSub_Api(object):
    def __init__(self, token, proxy=None) -> None:
        self.token = token # Sử dụng Token thay vì User/Pass
        self.proxies = None
        self.session = requests.Session()
        # Header chuẩn cho TDS
    ...
    def info(self):
        # API Check info profile qua token
        try:
            # Lấy thông tin profile để hiển thị tên và xu (dùng fields=profile)
            response = self.session.get(f'https://traodoisub.com/api/?fields=profile&access_token={self.token}', headers=self.headers, proxies=self.proxies).json()
            if 'data' in response:
                self.xu = response['data']['xu']
                self.name = response['data']['user']
    ...
    def facebook_configuration(self, id):
        # API cấu hình nick (Đặt nick) qua Token
        try:
            # Dùng API fields=run như yêu cầu
            url = f'https://traodoisub.com/api/?fields=run&id={id}&access_token={self.token}'
            response = self.session.get(url, headers=self.headers, proxies=self.proxies).json()
            # API fields=run trả về json có 'data' chứa 'id' và 'msg' nếu thành công
            if 'data' in response:
                if 'thành công' in response['data']['msg'] or response['data']['id'] == id:
                    return True
    ...
    def add_uid(self, id, g_recaptcha_response):
        # Vẫn cần login cookie TDS để add UID mới? 
        # API add_uid thường cần session web, nhưng nếu chỉ chạy tool thì có thể bỏ qua nếu đã thêm rồi.
        # Hàm này giữ nguyên logic cũ nhưng có thể sẽ lỗi nếu không login web.
        # Tuy nhiên yêu cầu của bạn chỉ là thay đổi Login bằng Token.
        # Nếu muốn add uid bằng token thì TDS API công khai không hỗ trợ add nick mới bằng token, chỉ run nick cũ.
        # Tạm thời giữ nguyên hoặc trả về False nếu không hỗ trợ.
        return False, "API Token không hỗ trợ thêm nick mới, vui lòng thêm trên web."
...
def Main():
    ...
    # --- PHẦN LOGIN ĐÃ SỬA ĐỔI (DÙNG TOKEN) ---
    while True:
        if os.path.exists('token_tds.txt'):
            with open('token_tds.txt', 'r') as f:
                tds_token = f.read().strip()
            # Khởi tạo class với Token
            tds = TraoDoiSub_Api(tds_token, current_proxy)
            profile = tds.info()
            try:
                if profile[0] == True:
                    print(f'{thanh} {luc}Nhập {red}[{vang}1{red}] {luc}Để Chạy Token {vang}{profile[1]}')
                    print(f'{thanh} {luc}Nhập {red}[{vang}2{red}] {luc}Nhập Token Trao Đổi Sub Mới')
                    thanhngang(70)
                    ...
                    raise Exception("Token Die")
            except:
                print(f'''{thanh} {luc}Trạng Thái Acc{trang}: {trang}[{red}DIE{trang}]\n{thanh} {luc}Tin Nhắn{trang}: {luc}Token Lỗi hoặc Hết Hạn''')
                os.remove('token_tds.txt')
        
        if not os.path.exists('token_tds.txt'):
            tds_token = input(f'{thanh} {luc}Nhập TOKEN TDS{trang}: ')
            thanhngang(70)
            with open('token_tds.txt', 'w') as f:
                f.write(f'{tds_token}')
    ...
    while True:
        ...
        for ck in listck:
            ...
            # --- CẤU HÌNH (ĐẶT NICK) DÙNG API FIELDS=RUN ---
            cauhinh = tds.facebook_configuration(uid)
            if cauhinh == True:
                print(f'{luc}Id Facebook{trang}: {vang}{uid2} {red}| {luc}Tên Tài khoản{trang}: {vang}{name}')
            else:
                # Nếu API token fields=run thất bại, có thể do nick chưa add vào hệ thống
                # Vì dùng token nên không auto add nick mới bằng captcha như cũ được dễ dàng
                print(f'{luc}Cấu Hình Thất Bại Id Facebook{trang}: {vang}{uid} {red}| {luc}Tên Tài khoản{trang}: {vang}{name}')
                # Nếu bạn muốn giữ logic auto add nick cũ thì cần login web, nhưng ở đây dùng token nên bỏ qua
                print(f'{red}Vui lòng thêm nick {uid} vào Traodoisub trước khi chạy!')
                listck.remove(ck)
                continue
            ...
            while True:
                ...
                # --- GIỮ NGUYÊN PHẦN LÀM NHIỆM VỤ TỪ ĐÂY TRỞ XUỐNG ---
                if '1' in nhiemvu:
